...
/* The qmp_avatar_armv7m_* prototypes come from the generated
 * qapi-commands-avatar-target.h now that the commands are guarded by
 * TARGET_ARM (visible to the qapi build) rather than the device-only
 * CONFIG_AVATAR (which the generated header never saw). The manual
 * forward declarations that used to live here are therefore redundant. */
